...
getLowestUnvisitedNodeInto macro reg
    ; Initialize counter
    mov ebx, 0
    ; In case no node is found, set
    ; the lowest cost node as nullptr
    mov reg, 0

compare:
    ; If all nodes are checked end macro
    cmp ebx, CURR_NODES
    ...
    ; If the node was visited, check the next node
    ...
    not_nullptr:
        ...
        ; Compare current node with the lowest cost node
        cmp [r12].Node.fCost, r11d 
        jl change_smallest
        jmp compare

    change_smallest:
        ;Change the lowest cost node to current node
        lea reg, [r12].Node
        jmp compare
...
getEuclideanDistanceInto macro reg, posFrom, posTo
    ...
    ; Move the result to the memory operand
    mov   [squareRoot], reg
    ; Load the squareRoot, convert it to floating point
    ; number and insert it on top of the stack.
    fild  [squareRoot]
    ; Calculate the square root of the element that is
    ; located on top of the stack.
    fsqrt
   
    ; Convert a floating point value from the top of the
    ; stack to an integer and then load it into squareRoot
    fistp squareRoot  
...
; void aStarSearch(int* board, Node* nodes, Path path, BoardDimension boardDimension);
; board             -> RCX  (Grid)
...
check_next_neighbour:
    ...
    je check_next_neighbour
        ...
        ; If it is not an obstacle and is in range, continue
        getNodeInto r11
        cmp r11, 0
        jne checkIfVisited
    ...
    checkIfVisited:
        ...
        jae check_next_neighbour
            ; New distance is smaller than the old one,
            ; so we can calculate new f cost
